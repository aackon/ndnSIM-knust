  ...
  static TypeId tid =
    TypeId("ns3::ndn::SpontaneousProducer")
      ...
      .AddAttribute("Frequency", "Frequency of data packets, if 0, then no spontaneous publish",
                    TimeValue(Seconds(2)), MakeTimeAccessor(&SpontaneousProducer::m_frequency),
                    MakeTimeChecker())
      .AddAttribute(
         "Signature",
...
SpontaneousProducer::SpontaneousProducer()
  ...
  , m_receivedpayload(0)
  , m_subDataSize (10)
  , m_dataRate(20)//  the default data rate value is set 
{
...
SpontaneousProducer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  NS_LOG_INFO("CONSUMER DATA RATE: " << interest->getDataRate() <<"TIME:"<< Simulator::Now()); //here is where the producer gets the features from the interest
 ...
 //🔁 Compute frequency from interest's DataRate
  double dataRate = interest->getDataRate();
  if (dataRate > 0) {
    double frequencySeconds = 1.0 / dataRate;
    m_frequency = Seconds(frequencySeconds);
    NS_LOG_INFO("Updated m_frequency to: " << m_frequency.GetSeconds() << " seconds");
  }
