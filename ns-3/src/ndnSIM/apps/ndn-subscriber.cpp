...
Subscriber::GetTypeId(void)
{
  static TypeId tid =
    TypeId("ns3::ndn::Subscriber")
      ...
      .AddAttribute("DataRate", "Rate at which subscriber wants to receive data packets (packets per second)", 
                    IntegerValue(1),
                    MakeIntegerAccessor(&Subscriber::m_dataRate), 
...
Subscriber::Subscriber()
    ...
    , m_dataRate(1)  // Added this line - initialize to default value
...
Subscriber::SendPacket()
{
  ...
  interest->setDataRate(m_dataRate);  // added the rate feature to the interest
