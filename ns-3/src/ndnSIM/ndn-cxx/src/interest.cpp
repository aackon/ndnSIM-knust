...
const uint32_t
Interest::getDataRate() const
{
   return m_dataRate;
}

Interest&
Interest::setDataRate(const uint32_t dataRate)
{
   m_dataRate = dataRate;
   m_wire.reset();
   return *this;
...
Interest::wireDecode(const Block& wire)
{
  ...
  // DataRate
  val = m_wire.find(tlv::DataRate);
  if (val != m_wire.elements_end()) {
    m_dataRate = readNonNegativeInteger(*val);
  }
  else {
    m_dataRate = 1;  // default value
  }
