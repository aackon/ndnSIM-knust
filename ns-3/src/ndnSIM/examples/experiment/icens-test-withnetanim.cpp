...
main(int argc, char* argv[])
{
  ...
  consumerHelper.SetAttribute("TxTimer", StringValue("20")); //resend subscription interest every 5 seconds
  ...
  AnimationInterface::SetConstantPosition(nodes.Get(0), 10, 50);   // Consumer position
  AnimationInterface::SetConstantPosition(nodes.Get(1), 100, 50);  // Router position  
  AnimationInterface::SetConstantPosition(nodes.Get(2), 190, 50);  // Producer position

  // Create animation interface - this will generate the XML file for NetAnim
  AnimationInterface anim("icens-test-animation.xml");
